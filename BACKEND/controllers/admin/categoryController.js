...
const editCategory = async (req, res) => {
  try {
   ...
   const updatedData= await Category.findByIdAndUpdate(catId, data, {
     ...
     if(updatedData){
         res.status(200).json({success:true,message:"Category updated",updatedData})
     }
  } catch (error) {
    res
    .status(error?.status || 500)
...
try {
   ...
   res.status(200).json({success:true,message:"Fetched category details",category})
} catch (error) {
    res
    .status(error?.status || 500)
    .json({ message: error.message || "Something went wrong" });
...
module.exports = { addCategory, showCategories, editCategory, listCategory ,showCategory};
